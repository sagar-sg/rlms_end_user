...
import com.rlms.network.webapi.APIRegisterUser;
...
//                    }
//                });
//andydev725@gmail.com
//                qwerty12345
        firebaseAuth.createUserWithEmailAndPassword(mobileStr+"@testuser.com", ""+getString(R.string.app_name))
                .addOnCompleteListener(this, new OnCompleteListener<AuthResult>() {
                    @Override
                    public void onComplete(@NonNull Task<AuthResult> task) {
                        progressDialog.dismiss();
                        ...
                        if(task.isSuccessful()){
                            ...
                            firebaseAuth.signInWithEmailAndPassword(mobileStr+"@testuser.com", ""+getString(R.string.app_name))
   ...
   * */
    private void callRegisterUserToServer(FirebaseUser currentUser) {
        ...
        APIRegisterUser apiRegisterUser = RetrofitBuilder.getClient().create(APIRegisterUser.class);

        RegisterUser registerUser = new RegisterUser();

        // hard core values
        registerUser.setAddress(""+localityWithPincode);
        registerUser.setAppRegId(currentUser.getUid());
        registerUser.setContactNumber(""+editTextMobile.getText().toString().trim());
        registerUser.setLongitude(latitude);
        registerUser.setLatitude(longitude);

        // call login api
        Call<RLMSAPIResponse> call = apiRegisterUser.callLoginUser(registerUser);

        Log.d(TAG, "apiRegisterUser url = " + call.request().url());
        ...
        call.enqueue(new Callback<RLMSAPIResponse>() {
            ...
            public void onResponse(Call<RLMSAPIResponse> call, Response<RLMSAPIResponse> response) {
                ...
                Log.d(TAG, "apiRegisterUser onResponse: statusCode " + statusCode);
                Log.d(TAG, "apiRegisterUser onResponse: message " + response.message());

                if (statusCode == 200) {

                    Toast.makeText(mContext,getString(R.string.registration_succes),Toast.LENGTH_SHORT).show();
                    RLMSAPIResponse rlmsapiResponse = response.body();
                    if(rlmsapiResponse != null){
                        Log.d(TAG, "apiRegisterUser connected successfully rlmsapiResponse = "+rlmsapiResponse.toString());

                        if(rlmsapiResponse.getResponse() != null && rlmsapiResponse.getResponse().length() != 0){
                            if(!rlmsapiResponse.getResponse().equalsIgnoreCase("Invalid contact number. Please enter your registered contact number with us.")){
                                User user = Parser.getParsedUser(rlmsapiResponse.getResponse());
                                user.setContactNumber(""+editTextMobile.getText().toString().trim());
                                new Preferences(mContext).storeUserDetails(user);
                            }else {
                                Toast.makeText(mContext, "" + rlmsapiResponse.getResponse(), Toast.LENGTH_LONG).show();
                            }


                        }else{
                            Toast.makeText(mContext, "" + mContext.getString(R.string.failed_to_register), Toast.LENGTH_LONG).show();
                        }
                    }
                } else {
                    Log.d(TAG, "apiRegisterUser statusCode is not 200" + statusCode);
                    Toast.makeText(mContext, "" + mContext.getString(R.string.failed_to_register), Toast.LENGTH_LONG).show();
                }
                startActivity(new Intent(getApplicationContext(), MainActivity.class));
                finish();
            ...
            public void onFailure(Call<RLMSAPIResponse> call, Throwable t) {
                Log.d(TAG, "apiRegisterUser on failure wifi connect  = " + t.getMessage());
                progressDialog.dismiss();
